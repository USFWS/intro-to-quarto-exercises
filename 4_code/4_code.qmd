...
Site_map <- ggplot() +
  ...
  theme(panel.background = element_rect(fill = "#c9e9fa"), 
        ...
        panel.border = element_rect(colour = "black", 
                                    fill = NA, 
                                    size = .5)) # Gives the plot a plain standard appearance

Site_map # calls the map to be plotted
```

```{r}
#| label: fig-data_manipulation
...
```{r}
#| label: tbl-observation_summary
#| fig-cap: !expr paste0("Seasonal Western Snowy Plover observational counts at the ", params$Site, " survey location.")
...
catch_time_data <- data %>% 
  ...
  mutate(Mcpue = Males/Duration_Hours,
...
### The Plot
cpue_plot <- ggplot(catch_time_data,
                    aes(x = Month, y = meanCPUE)) + 
  geom_col(aes(fill = Maturity),
           color = "black")+
